...
Given(
  ...
  async function (this: CustomWorld, name: string, email: string) {
    ...
    await this.accountRegistrationDSL.ensureUserExists({
      name,
      email,
...
When(
  "a user registers with valid details:",
  async function (this: CustomWorld, dataTable: any) {
    if (!this.accountRegistrationDSL) {
      throw new Error("DSL not initialized");
    }
    const [registrationData] = dataTable.hashes();
    const isTeamLeader = registrationData.IsTeamLeader === "true";
    const details: RegistrationDetails = {
      name: registrationData.Name,
      email: registrationData.Email,
      password: registrationData.Password,
      roleId: isTeamLeader ? 1 : 2,
      isTeamLeader,
    };
...
When(
  ...
  async function (this: CustomWorld, dataTable: any) {
    ...
    const isTeamLeader =
      registrationData.IsTeamLeader !== undefined
        ? registrationData.IsTeamLeader === "true"
        : true;
...
Then(
  ...
  async function (this: CustomWorld) {
    ...
    const registrationResult =
      await this.accountRegistrationDSL.getRegistrationResult();
    expect(registrationResult.success).toBe(true);
  }
);

Then(
  "a confirmation should be sent to {string}",
  async function (this: CustomWorld, email: string) {
    if (!this.accountRegistrationDSL) {
      throw new Error("DSL not initialized");
    }
    const confirmationSent =
      await this.accountRegistrationDSL.verifyConfirmationSent(email);
...
Then(
  ...
  async function (this: CustomWorld, expectedReason: string) {
    if (!this.accountRegistrationDSL) {
      throw new Error("DSL not initialized");
    }
    const registrationResult =
